...
| **Suggestion phraser** | `SuggestionPhraser`: turns an `OwnershipSuggestion` into human text. The default `TemplateSuggestionPhraser` is deterministic and factual; `AiSuggestionPhraser` (phase 8) rewrites it via the `SuggestionTextGenerator` seam, falling back to the factual text and never changing the named owner (D14). |
