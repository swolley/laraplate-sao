...
The integration layer's foundation is in place, and the first concrete external driver — **Redmine** (`Modules\SAO\Drivers\External\RedmineDriver`, registered by default in `config('sao.drivers.registered')`) — ships: it serves the `issues` capability over the documented Redmine REST API (`X-Redmine-API-Key` auth, the binding's `remoteIdentifier` as the Redmine `project_id`) and passes the `issues` conformance suite against a network-free `Http::fake()`. A **driver** is registered code (`Modules\SAO\Drivers`); a **connection** (`Connection` model) is a configured instance of it. The registry is **open**: `DriverRegistry` is a singleton populated from `config('sao.drivers.registered')` and by any provider's `boot()` — adding a provider never requires editing SAO. Duplicate keys throw so a collision surfaces at boot.
...
-   Phase 3 — driver framework, connections, capabilities and the first external issue tracker (3a foundation **done**: registry, contracts, `Connection`, credential resolver, conformance suite; 3b **done**: `ProjectBinding`, `TicketLink`, the internal issues driver, `IssueSyncService`, and the first concrete external driver — **Redmine** — passing the issues conformance suite over an `Http::fake()`. Live-instance verification and webhook/push ingest remain follow-ups)
