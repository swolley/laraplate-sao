...
/**
 * The `issues` driver for Gitea, over its GitHub-shaped REST API. Authentication
 * is a token in the `Authorization: token <token>` header; the binding's
 * `remoteIdentifier` is `owner/repo`. Pagination follows the `Link` header's
 * `rel="next"`. Gitea has no native priority, and its status is the issue
 * `state` (open/closed) mapped through the binding map. Pull requests share the
...
final readonly class GiteaDriver implements DriverInterface, IssuesCapability
{
    ...
    public function configurationSchema(): DriverConfigurationSchema
    {
        return new DriverConfigurationSchema([
            new ConfigurationField('token', 'string', 'Gitea access token', required: true, secret: true),
    ...
    public function healthCheck(ConnectionContext $context): HealthCheckResult
    {
        if ($context->baseUrl === null) {
            return HealthCheckResult::unhealthy('No base URL configured for the Gitea connection.');
        }

        try {
            $response = $this->client($context)->get('/version');
        ...
        return $response->successful()
            ...
            : HealthCheckResult::unhealthy("Gitea returned HTTP {$response->status()}.");
    }
    ...
    {
        $response = $this->connection($context)->post($this->repoPath($context) . '/issues', $this->toGiteaIssue($attributes));
    ...
    {
        $response = $this->connection($context)->patch($this->repoPath($context) . "/issues/{$remoteId}", $this->toGiteaIssue($attributes));
     ...
     * @return array<string, mixed>
     */
    private function toGiteaIssue(array $attributes): array
    {
        $issue = [];
